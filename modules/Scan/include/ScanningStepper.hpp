...
/**
 * @class ScanningStepper
 * @brief Implementation of Class IScanning.
 * 
 */
class ScanningStepper : public IScanning{
 public:
  /**
   * @brief Construct a new ScanningStepper object.
   * 
   * @param clientStepper shared pointer to IMotor Class.
   * @param clientSensors shared pointer to ISensors Class.
   * @param clientPostProcessing shared IPostProcessing to IMotor Class.
   */
  ScanningStepper(std::shared_ptr<IMotor> clientStepper,
 ...
 private:
  ...
  std::filesystem::path pathToPlotScanScript_ = pathToProjDirectory_ / "modules\\Devices\\scripts\\Scans\\plotScanStepper.py";  /**< Path to the script used to plot the data logged during the scan. */
