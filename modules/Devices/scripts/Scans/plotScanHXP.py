...
def plotScan():
    ...
    The plot displays the signal intensity (y-axis) as a function of the position of one of the axis of the hexapod (x-axis) using two line plots: one for the raw data and the other for the smoothed data obtained by applying a Savitzky-Golay filter.
    
    Args:
    sys.argv[1] (string): filename of the input .csv file xray sensor data.
    sys.argv[2] (int)   : hexapod axis scanning.

    Note:
    The function relies on a module called 'moduleFunctions'.
    """
    logFileName = sys.argv[1]
    axisToScan = int(sys.argv[2])
    ...
    g1=sns.lineplot(y=dataframe.iloc[:, 0],
                    x=dataframe.iloc[:, axisToScan], ax=ax1, color = 'blue', linewidth=2, markers=True, marker='o')
    g=sns.lineplot(y=dataframe_smoothed.iloc[:, 0],
                   x=dataframe.iloc[:, axisToScan], ax=ax1, color = 'orangered', linewidth=2, markers=True, marker='o')
    plt.ylabel('Signal Intensity (arb. units)')
    plt.xlabel('Axis n.' + str(axisToScan) + ' Position')
