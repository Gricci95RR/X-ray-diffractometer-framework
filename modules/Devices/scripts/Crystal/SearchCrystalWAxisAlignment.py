...
def searchBraggPeak():
    """
    Brief:
    This function calculates the Bragg's Peak angle of a given X-ray sensor data. The X-ray sensor data is read from a .csv file
    ...
    g1=sns.lineplot(y=dataframe['X-Ray Sensor Data'], x=dataframe['HXP W-Axis'], ax=ax1, color = 'blue', linewidth=2, markers=True, marker='o')
    g=sns.lineplot(y=dataframe_smoothed['X-Ray Sensor Data'], x=dataframe['HXP W-Axis'], ax=ax1, color = 'orangered', linewidth=2, markers=True, marker='o')
    g.axvline(center_angle, color = "lime")
    ...
    plt.xlabel('Crystal - W Axis Position (deg)')
    plt.title("Center W angle: " + str(center_angle)[:5] + "°")
    ax1.legend({"X-Ray Sensor Data": dataframe["X-Ray Sensor Data"],
                "Smoothed X-Ray Sensor Data": dataframe["X-Ray Sensor Data"]},
                loc = "lower right",
                title_fontsize = "1")
    
    plt.savefig("W scan" + str(now) + ".png", dpi = 200)
    plt.show()

    # Save result in a .csv file
    list_position_in_idmax_X_Ray_Sensor_Data_sm = [center_angle]
    dictionary = {
        'Center W Angle - W Axis Hexapod (deg)': list_position_in_idmax_X_Ray_Sensor_Data_sm,
        }
    df = pandas.DataFrame(dictionary)
    df.to_csv(index=False, path_or_buf = save_path)

    return float(center_angle)
